...
export class DetailsComponent implements OnInit {
  ...
  constructor(
    ...
    // Decrypting the id from the URL
    let temp = this.route.snapshot.queryParams['id'];
    console.log("temp:", temp);
    this.invoiceToken = atob(temp);
    console.log("invoiceToken:", this.invoiceToken);
  }

//   ngOnInit(): void {
//     console.log("show loader");

//     // Retrieve user ID from GlobalService
//     this.userId = this.globalService.getUserId();
//     console.log('User ID in DetailsComponent:', this.userId); // Debugging
...
//     console.log("id:", this.invoiceToken);
...
//             isvalidated: res['isvalidated'] === '1' ? true : false
//           });
...
//       let dataobj: any = {
//         'invoice-date': this.invoiceForm.get('invoicedate')?.value,
//         'invoiceno': this.invoiceForm.get('invoiceno')?.value,
//         'grandtotal': this.invoiceForm.get('grandtotal')?.value,
//         'barcode': this.invoiceForm.get('barcode')?.value,
//         'validate-grn-no': this.invoiceForm.get('validate-grn-no')?.value,
//         'multi-grn-no': this.invoiceForm.get('multi-grn-no')?.value,
//         'invoice-type': this.invoiceForm.get('invoice-type')?.value,
//         'currency': this.invoiceForm.get('currency')?.value,
//         'service-entry-year': this.invoiceForm.get('service-entry-year')?.value,
//         'service-entry-no': this.invoiceForm.get('service-entry-no')?.value,
//         'invoice-status': this.invoiceForm.get('invoice-status')?.value,
//         'comment': this.invoiceForm.get('comment')?.value,
//         'lastupdatedBy': this.userId, // Assigning userId from GlobalService
//         'isvalidated': ((document.getElementById("isvalidated") as HTMLInputElement).checked ? "1" : "0")
//       };
//       console.log("Logged user ID:", this.userId); // Debugging
...
editInvoiceDataGet() {
  this.submitted = false;
  const timeZone = Intl.DateTimeFormat().resolvedOptions().timeZone;
  if (this.invoiceToken && this.invoiceToken !== "") {
    this.service.getInvoiceDetails(btoa(this.invoiceToken)).subscribe({
      next: res => {
